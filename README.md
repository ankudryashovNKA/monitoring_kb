...
`Monitoring KB` — это сервер мониторинга узлов (node monitoring) с локальным агентом и встроенным веб-дашбордом в одном FastAPI-приложении.
...
- даёт UI-вкладку LLM, которая обращается к локально установленному Ollama через Python SDK (`gemma4:e4b`).
- даёт возможность автоматизации типовых проблем на удалённых узлах, с помощью привязки исполняемых скриптов к триггерам
...
Назначение: правила срабатывания по метрикам и автоматизация решения типовых инцидентов.

- создаёте правило: node + метрика + оператор (`>`/`<`) + threshold;
- при создании триггера можно привязать исполняемый скрипт: он запускается при срабатывании триггера и помогает автоматически устранять типовые проблемы на удалённых узлах;
...
1. Сложите рабочие автоматизации в `./scripts` на удалённом узле, где запускается агент, путь указан относительно агента.
